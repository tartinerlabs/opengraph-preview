...
- `check-platforms.ts` — which checks change which platform card; drives the tab issue dots (unit-tested)
- `app.tsx`, `popup-header.tsx`, `preview-tabs.tsx`, `platform-previews.tsx`, `preview-image.tsx` — UI states and one component per platform surface; the tab strip (Image, X, Facebook, LinkedIn, Slack, Discord, WhatsApp, Reddit) scrolls horizontally, with Checks and Raw tags below it
